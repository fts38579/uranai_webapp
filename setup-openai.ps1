...
if (-not $ApiKey) {
  $ApiKey = Read-PlainSecret 'OpenAI API key を入力してください'
}
...
if (-not $ApiKey.StartsWith('sk-')) {
  Write-Error 'OpenAI API key の形式に見えません。通常は sk- から始まります。'
  exit 1
}
...
Set-Or-AddEnvLine -Lines $lines -Key 'OPENAI_API_KEY' -Value $ApiKey
...
Write-Host "OpenAI API key を保存しました。" -ForegroundColor Green
...
try {
  ...
  if ($json.openaiKeyConfigured) {
    Write-Host "GPT 接続準備OK: http://127.0.0.1:$Port/uranai-v5.html" -ForegroundColor Green
  } else {
    Write-Warning 'server.js は起動しましたが、OPENAI_API_KEY を認識できていません。'
  }
