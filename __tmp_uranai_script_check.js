...
const ORACLE={
  1:{name:"The Guide",msg:"自分の信じる方向を示し、迷っている人や状況に新しい道筋を作ってください。"},
  2:{name:"The Supporter",msg:"前に出るよりも支える役割に徹することで、物事は静かに好転していきます。"},
  3:{name:"The Innocent",msg:"難しく考えすぎず、純粋な気持ちと楽しさを大切に行動してください。"},
  4:{name:"The Diligent",msg:"焦らず努力を積み重ねることで、揺るがない基盤が築かれていきます。"},
  5:{name:"The Adventurer",msg:"未知の選択を恐れず、新しい体験の中に成長のチャンスを見つけてください。"},
  6:{name:"The Caregiver",msg:"誰かを思いやる行動が、巡り巡って自分の安心と愛を深めます。"},
  7:{name:"The Artisan",msg:"自分の感性や技術を磨き、内面から生まれる価値を形にしてください。"},
  8:{name:"The Warrior",msg:"覚悟を決めて行動することで、現実はあなたの味方になります。"},
  9:{name:"The Sage",msg:"経験から得た知恵を使い、物事を広い視点で受け止めてください。"},
  10:{name:"The Great Power",msg:"新しい力の流れが始まっているため、自分の可能性を制限しないでください。"},
  11:{name:"The Inspired One",msg:"直感やひらめきを手がかりにすると、重要な手がかりが見えてきます。"},
  12:{name:"The Harmonizer",msg:"対立や違いを調整し、全体のバランスを整える役割を意識してください。"},
  13:{name:"The Wise King",msg:"冷静な判断と責任ある行動によって、周囲に安定をもたらしてください。"},
  14:{name:"The Transformer",msg:"変化を受け入れ、自分自身をより高い形へと更新してください。"},
  15:{name:"The Servant",msg:"誰かの役に立つ行動が、結果として大きな意味を生みます。"},
  16:{name:"The Perceptive One",msg:"物事の本質を見抜くために、表面ではなく内側を観察してください。"},
  17:{name:"The Benefactor",msg:"与えることを惜しまない姿勢が、豊かな流れを呼び込みます。"},
  18:{name:"The Seeker",msg:"まだ見えていない答えを求めて、学びと探求を続けてください。"},
  19:{name:"The Unwavering One",msg:"困難があっても信念を曲げず進むことで道は開かれます。"},
  20:{name:"The Unifer",msg:"人や意見をつなげることで、新しい可能性が生まれます。"},
  21:{name:"The Completer",msg:"ひとつの流れが完成に近づいているため、最後まで丁寧に仕上げてください。"},
  22:{name:"The Carisma",msg:"自分の存在感や影響力を意識し、それを前向きな方向に使ってください。"},
  23:{name:"The Flowrider",msg:"状況を無理に変えようとせず、今の流れを上手に乗りこなしてください。"},
  24:{name:"The Gracebearer",msg:"優しさと品位ある行動が、周囲の空気を整えていきます。"},
  25:{name:"The Wayfarer",msg:"自分のペースで歩み続けることで、本当に進むべき道が見えてきます。"},
  26:{name:"The Trailblazer",msg:"まだ誰も進んでいない道を恐れず切り開いてください。"},
  27:{name:"The Gatewalker",msg:"人生の節目に立っているため、新しい段階へ進む準備をしてください。"},
  28:{name:"The Resonator",msg:"共鳴する人や環境を選ぶことで、運気の流れが整います。"},
  29:{name:"The Visionweaver",msg:"未来のビジョンを具体的に描くことで現実が動き始めます。"},
  30:{name:"The Creator",msg:"自分の想像力を信じ、形にすることを恐れないでください。"},
  31:{name:"The Architect",msg:"長期的な視点で計画を立て、現実的に構築していきましょう。"},
  32:{name:"The Collaborator",msg:"信頼できる仲間と力を合わせることで、成果が大きくなります。"},
  33:{name:"The Awakened",msg:"高い視点から物事を見て、周囲の成長を助ける存在になってください。"},
};
...
function buildLifePatternPlainText(lp=LP){
  ...
  return `${card?.msg||'自分らしさが出やすい動き方があります。'}${card?.name?` その人らしさの軸として「${card.name}」のテーマが重なりやすい傾向です。`:''}`;
}
...
let RUNTIME_HEALTH={checked:false,reachable:false,openai:false,anthropic:false,error:'',mode:''};
...
function bootMemberMode(){
  try{
    if(MEMBER_PREVIEW_PARAM) localStorage.setItem(MEMBER_STORAGE_KEY,'1');
    MEMBER_MODE=localStorage.getItem(MEMBER_STORAGE_KEY)==='1';
  }catch(e){
    MEMBER_MODE=MEMBER_PREVIEW_PARAM;
  }
}
...
function renderPremiumEntrySection(){
  ...
  if(!el) return;
  const statusLabel=isMemberActive()?'記録機能を表示中':'深掘り鑑定は利用可';
  el.innerHTML=`
    <div class="premium-entry-head">
      <div>
        ...
        <div class="premium-entry-meta">
          ...
          <div class="premium-entry-pill">${escapeHtml(statusLabel)}</div>
        </div>
      </div>
    </div>
    ...
    <div class="premium-entry-actions">
      <button class="today-cta" onclick="startFlow('paid')">深掘り鑑定を受ける</button>
      <button class="premium-entry-subbtn" onclick="startFlow('free')">まずは無料で試す</button>
    </div>`;
}

function toggleMemberPreview(){
  MEMBER_MODE=!MEMBER_MODE;
  try{localStorage.setItem(MEMBER_STORAGE_KEY,MEMBER_MODE?'1':'0');}catch(e){}
  renderHomeVault();
  renderMemberFollowupSection();
  showToast(MEMBER_MODE?'会員プレビューを有効化しました':'会員プレビューを解除しました');
}
...
function buildFormattedReadingMarkup(text='',kind='default'){
  const configs={
    len:{kicker:'MAIN MESSAGE',title:'ルノルマンカード鑑定'},
    orc:{kicker:'MAIN MESSAGE',title:'オラクルカード鑑定'},
    foundationdeep:{kicker:'FOUNDATION',title:'基礎鑑定の統合詳細'},
    integration:{kicker:'FINAL ANSWER',title:'いまの答え'},
    default:{kicker:'MESSAGE',title:'鑑定結果'},
  };
  ...
  if(!sections.length){
    ...
    return `<div class="reading-rich-shell kind-${kind}">
      <div class="reading-rich-hero">
        <div class="reading-rich-kicker">${config.kicker}</div>
...
function buildReadingOutputFormatGuide(kind='len'){
  if(kind==='len'){
    return [
      ...
      '■ 意識しておくこと',
      '■ ズバリ次の行動',
      '各見出しでは、最初の1〜2文を結論として自然な日本語で短く言い切ってください。',
      '比喩を増やしすぎず、相談者が読み返して判断材料にできる文体にしてください。'
    ].join('\n');
  }
  if(kind==='orc'){
    return [
      ...
      '■ 整え方と未来へのヒント',
      '各見出しでは、最初の1〜2文を結論として自然な日本語で短く言い切ってください。',
      '「あなたの本質」は四柱推命・姓名判断・体験エピソード診断から見える根っこの性質だけを書くこと。',
      '励ましだけで流さず、整え方と行動につながる具体性を残してください。'
    ].join('\n');
  }
  if(kind==='integration'){
    return [
      ...
      '■ いま優先すること',
      '■ 行動計画',
      '「最終結論」は最初の2文で、進める・いったん止まる・条件付きで進めるのどれかが伝わるように言い切ってください。',
      '「いま優先すること」では、進めてよい目印と見直すべき目印が分かるように書いてください。',
      '「行動計画」は今日から7日以内にやることを3つ、1行ずつ行動から始まる短い文で書いてください。'
    ].join('\n');
  }
...
function persistCurrentReading(){
  ...
  history.unshift(buildCurrentReadingRecord());
...
function renderHomeVault(){
  ...
  renderMemberStatusBlock();
  renderPremiumEntrySection();
}
...
function renderMemberStatusBlock(){
  ...
  if(!el) return;
  const previewBtn=DEV_MODE?`<button class="vault-link" onclick="toggleMemberPreview()">${isMemberActive()?'会員プレビューを解除':'会員プレビューを有効化'}</button>`:'';
  if(isMemberActive()){
    el.innerHTML=`
      <div class="member-state active">記録機能を表示中</div>
      <div class="vault-desc">一度読んで終わりではなく、前回の流れを引き継ぎながら、自分の進路を育てていける状態です。価値は「当たること」より、「使うほど自分の羅針盤がはっきりすること」にあります。</div>
  ...
  el.innerHTML=`
    <div class="member-state inactive">公開準備中</div>
    <div class="vault-desc">深掘り鑑定自体は使えます。その先の、読み返しや記録の蓄積をひとつづきで使える形を準備しています。</div>
      <div class="member-benefits">
...
function renderResultUpgradePanel(){
  ...
  const items=[
    {kicker:'相談全体',body:'断片的な答えではなく、相談全体の流れをつかむ。'},
    {kicker:'核心',body:'励ましだけで終わらず、問題の核心と次の一手まで整理する。'},
    {kicker:'記録価値',body:'読み返すたびに、自分の傾向と判断パターンが見えてくる。'},
    {kicker:'今回のテーマ',body:`${themeLabel}を表面的な印象で終わらせず、判断材料として持ち帰れるところまで深掘りする。`},
  ];
  ...
  el.innerHTML=`
    <div class="upgrade-head">
      <div>
        ...
        <div class="upgrade-title">ここから先は、向きを進路に変える読みです</div>
        <div class="upgrade-copy">いまの無料結果は入口です。${escapeHtml(themeLabel)}を、いま出ているカードを軸にそのまま深掘りすると、当たっている気がするで終わらず、迷いの正体と次に取る動きまで見えてきます。</div>
        <div class="upgrade-meta">
          <div class="upgrade-price">
            <div class="upgrade-price-label">継続価格の想定</div>
            <div class="upgrade-price-value">${escapeHtml(MEMBERSHIP_PLAN.price)}</div>
          </div>
          <div class="upgrade-note">深掘り鑑定はこのまま続けられます。読み返しや記録機能は準備中です。</div>
        </div>
      </div>
      <div class="upgrade-spotlight">
        <div class="upgrade-spotlight-label">無料との違い</div>
        <div class="upgrade-spotlight-value">向きから、進路へ</div>
        <div class="upgrade-spotlight-copy">無料で出た3枚とオラクル1枚を起点に、追加カードだけを展開して深めます。いまの読みを別物にせず、そのまま進路判断へつなげるための深掘りです。</div>
      </div>
    </div>
    ...
    <div class="upgrade-actions">
      <button class="nav-btn nav-btn-primary" onclick="upgradeCurrentReadingToPaid()">この結果の続きを深く読む</button>
      <button class="vault-link" onclick="startFlow('paid')">別テーマで深掘り鑑定を受ける</button>
    </div>`;
}
...
function upgradeCurrentReadingToPaid(){
...
function clearReadingHistoryData(){
...
function startLenShuffle(){
  ...
  document.getElementById('len-inst').textContent='落ち着いて、止めたいと思った瞬間で止めてください';
}
...
function stopLen(){
  ...
  document.getElementById('len-inst').textContent='いまの流れに合うカードを整えています…';
...
function startOrcShuffle(){
  ...
  document.getElementById('orc-inst').textContent='落ち着いて、止めたいと思った瞬間で止めてください';
}
...
function stopOrc(){
  ...
  document.getElementById('orc-inst').textContent=`今の気持ちに近いカードを ${count} 枚選んでください。色や模様の違いで選んで大丈夫です。`;
...
function buildClarifyQuestions(){
  ...
  const questions=[
    ...
    buildBlockerClarifyQuestion(ctx),
  ];
  if(['恋愛','人間関係'].includes(ctx.category)){
    questions.push(buildFeelingsClarifyQuestion(ctx));
    if(ctx.externalCard||ctx.peopleCard||ctx.themeShort) questions.push(buildExternalClarifyQuestion(ctx));
  }else{
    questions.push(buildExternalClarifyQuestion(ctx));
  }
  questions.push(buildOutcomeClarifyQuestion(ctx));
  if(ctx.themeShort||ctx.blockerCard||ctx.ambiguityCard||['仕事','金運'].includes(ctx.category)){
...
■ 背景と現状
ここに至る背景と、いま現実に起きていることを一続きで整理する。冒頭1〜2文で結論を言い切る。

■ 意識しておくこと
見落としやすい本音、認識と現実のズレ、分岐点を言語化し、本人がいま意識しておくべき点を書く。
${focus.isDualConcern?`恋愛と仕事が両方あるので、必要なら「恋愛では」「仕事では」と分けて整理する。`:''}

■ ズバリ次の行動
抽象論を禁止。今日から7日以内にできる動きを3つ以上、実行しやすい順で書く。

合計${is9?'1600字以上':is3?'850字以上':'450字以上'}。`;
  // 絞り込み回答があれば注入
...
${buildReadingOutputFormatGuide('len')}`;
  ...
  try{
    const res=await callAI(userPrompt,is9?4096:(is3?1400:500),systemPrompt,{
...
async function runOrcReading(){
  ...
  const orcInfo=SEL_ORC.map((id,i)=>`${orcLabels[i]||''}：No.${id}「${ORACLE[id].name}」\nメッセージ：${ORACLE[id].msg}`).join('\n');
  const lpCard=LP?ORACLE[LP]:null;
  const lpGuide=LP
...
- ${is3?'左のカードは背景、中のカードは現状、右のカードは実現しやすい未来とそのための助言として扱う':'1枚引きでは、いま抱えている気持ちと整え方を一本で読む'}
- ${lpGuide}
...
■ 背景と現状
ここまでの流れと、いま抱えすぎていることを一続きで整理する。冒頭1〜2文で結論を言い切る。

■ あなたの本質
四柱推命・姓名判断・体験エピソード診断から見える、その人の根っこの性質だけを書く。慰めではなく、本人が自分の扱い方を理解できる密度で書く。

■ 整え方と未来へのヒント
気持ちの立て直し方と、望む未来へ近づくための現実的な動きを一つの流れで書く。今日から7日以内にできる動きを2つ以上まじえて具体的に書く。

合計${is3?'1100字以上':'380字以上'}。`;
...
${buildReadingOutputFormatGuide('orc')}`;
  ...
  try{
    const res=await callAI(userPrompt,is3?4096:400,systemPrompt,{
      taskKey:PLAN==='paid'?'paid':'free',
...
${lenSpreadContext.diagonalDetails}`
  ...
  const orcFull=SEL_ORC.map((id,i)=>{
    const o=ORACLE[id];
    return`${getOrcSpreadLabel(i,SEL_ORC.length)}：No.${id}「${o.name}」→ ${o.msg}`;
  }).join('\n');
...
■ 最終結論
相談者が最初に読む2文で、いま進めるべきか、いったん止まるべきか、条件付きで進めるべきかが分かるように書く。

■ いま優先すること
何を確認できたら進めてよくて、何が見えたら切り替えるべきかを書く。判断の目印として読める密度にする。
${focus.isDualConcern?`恋愛と仕事が両方あるので、必要なら二つに分けて整理する。`:''}

■ 行動計画
今日から7日以内にやることを3つ、1行ずつ、行動から始まる短い文で書く。
...
${buildReadingOutputFormatGuide('integration')}`;
  ...
  try{
    const res=await callAI(prompt,1200,systemPrompt,{
      taskKey:PLAN==='paid'?'paid':'free',
...
${sourceContext}`;
  ...
  return await callAI(prompt,2200,systemPrompt,{
    taskKey:'dossier',
...
読み手は占いの仕組みを知らない一般ユーザーなので、専門用語なしで自然に読めることを最優先してください。`;
  ...
  try{
    const draft=await callAI(prompt,2200,systemPrompt,{
      taskKey:'dossier',
      images:buildCardImageRefs('all','dossier'),
    });
...
function renderMemberFollowupSection(){
  ...
  stateEl.textContent=isMemberActive()?'追加質問を表示中':'公開準備中';
  copyEl.textContent=isMemberActive()
    ?'結果を読んだあとに残る「あと1つだけ聞きたいこと」を追加で見られます。たとえば相手の気持ち、この7日でやること、動く時期などを絞って深めるための欄です。'
    :'深掘り鑑定はこの画面まで使えます。公開後は、今回の結果を土台にして、あと1つだけ聞きたいことを追加で見られるようになります。';

  if(!isMemberActive()){
    noteEl.style.display='block';
    noteEl.textContent='いまはまだ使えません。公開後は、いまの結果を土台にして、気になる1点だけを追加で見られるようになります。';
    actionsEl.innerHTML='';
  }else{
...
function loadApiKey(){
  ...
  const saved=sessionStorage.getItem('uranai-apikey')||'';
...
async function loadServerHealth(silent=false){
  if(!silent){
    RUNTIME_HEALTH={checked:false,reachable:false,openai:false,anthropic:false,error:'',mode:''};
    renderRuntimeStatus();
  }
  if(!canUseProxy()){
    RUNTIME_HEALTH={checked:true,reachable:false,openai:false,anthropic:false,error:'LOCAL_FILE',mode:''};
    renderRuntimeStatus();
    return;
  }
  try{
    ...
    RUNTIME_HEALTH={checked:true,reachable:false,openai:false,anthropic:false,error:'FETCH_FAILED',mode:''};
  }
  renderRuntimeStatus();
}
